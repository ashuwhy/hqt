	...
	model_cc_tmp_stream << " *  ";
{
    ...
    if(val.type()==typeid(const char *)){
        model_cc_tmp_stream<<*(std::any_cast<const char *>(&val));
    }else if(val.type()==typeid(std::string)||val.type()==typeid(const std::string)){
        model_cc_tmp_stream<<*(std::any_cast<const std::string>(&val));
    }
}
...
//                model_cc_tmp_stream<<"            "<<col.colValName_<<"_=std::make_shared<::trantor::Date>(::trantor::Date(946656000000000).after(daysNum*86400));\n";
 ...
 //               model_cc_tmp_stream<<"            "<<col.colValName_<<"_=std::make_shared<::trantor::Date>(::trantor::Date(946656000000000).after(daysNum*86400));\n";
            ...
            {
                model_cc_tmp_stream<<"            auto str = r[\""<<col.colName_<<"\"].as<std::string_view>();\n";
...
model_cc_tmp_stream<<i;
 ...
 //               model_cc_tmp_stream<<"            "<<col.colValName_<<"_=std::make_shared<::trantor::Date>(::trantor::Date(946656000000000).after(daysNum*86400));\n";
            ...
            {
                model_cc_tmp_stream<<"            auto str = r[index].as<std::string_view>();\n";
                model_cc_tmp_stream<<"            if(str.length()>=2&&\n";
                model_cc_tmp_stream<<"                str[0]=='\\\\'&&str[1]=='x')\n";
                model_cc_tmp_stream<<"            {\n";
                model_cc_tmp_stream<<"                "<<col.colValName_<<"_=std::make_shared<std::vector<char>>(drogon::utils::hexToBinaryVector(str.data()+2,str.length()-2));\n";
...
model_cc_tmp_stream<<"\n";
    for(size_t i=0;i<cols.size();i++)
    {
        ...
        if(!col.colType_.empty())
        {
            ...
            model_cc_tmp_stream<<"{\n";
            model_cc_tmp_stream<<"    static const "<<col.colType_<<" defaultValue = "<<col.colType_<<"();\n";
            ...
            if(col.colType_=="std::vector<char>")
            {
                ...
                model_cc_tmp_stream<<"{\n";
                model_cc_tmp_stream<<"    static const std::string defaultValue = std::string();\n";
                model_cc_tmp_stream<<"    if("<<col.colValName_<<"_)\n";
...
if(!col.notNull_){
	...
	model_cc_tmp_stream << "            }\n";
                ...
                {
	model_cc_tmp_stream << "            if(pJson.isString() && std::strlen(pJson.asCString()) > ";
...
model_cc_tmp_stream<<alias;
	model_cc_tmp_stream << "(const DbClientPtr &clientPtr) const {\n";
	model_cc_tmp_stream << "    static const std::string sql = \"select * from ";
...
model_cc_tmp_stream<<relationship.targetKey();
	...
	model_cc_tmp_stream << "\";\n";
	model_cc_tmp_stream << "    Result r(nullptr);\n";
	model_cc_tmp_stream << "    {\n";
	model_cc_tmp_stream << "        auto binder = *clientPtr << sql;\n";
	model_cc_tmp_stream << "        binder << *";
model_cc_tmp_stream<<nameTransform(relationship.originalKey(), false);
	model_cc_tmp_stream << "_ << Mode::Blocking >>\n";
	model_cc_tmp_stream << "            [&r](const Result &result) { r = result; };\n";
	model_cc_tmp_stream << "        binder.exec();\n";
	model_cc_tmp_stream << "    }\n";
	model_cc_tmp_stream << "    if (r.size() == 0)\n";
	model_cc_tmp_stream << "    {\n";
	model_cc_tmp_stream << "        throw UnexpectedRows(\"0 rows found\");\n";
	model_cc_tmp_stream << "    }\n";
	model_cc_tmp_stream << "    else if (r.size() > 1)\n";
	model_cc_tmp_stream << "    {\n";
	model_cc_tmp_stream << "        throw UnexpectedRows(\"Found more than one row\");\n";
	model_cc_tmp_stream << "    }\n";
	model_cc_tmp_stream << "    return ";
model_cc_tmp_stream<<relationshipClassName;
